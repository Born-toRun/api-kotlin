...
import kr.kro.btr.infrastructure.RedisGateway
...
class RecentSearchKeywordService(
    private val redisGateway: RedisGateway
) : RecentSearchKeywordPort {

    @Async
    @Transactional
    override fun removeAll(userId: Long) {
        redisGateway.removeAll("$RECENT_SEARCH_KEYWORD_KEY_PREFIX$userId")
    }

    @Async
    @Transactional
    override fun removeKeyword(userId: Long, searchKeyword: String) {
        redisGateway.removeValue("$RECENT_SEARCH_KEYWORD_KEY_PREFIX$userId", searchKeyword)
    }
    ...
    override fun add(userId: Long, searchKeyword: String) {
        ...
        redisGateway.add(key, searchKeyword)

        val recentSearchKeywords = redisGateway.getList(key).toMutableList()
    ...
    override fun search(userId: Long): List<Any> {
        return redisGateway.getList("$RECENT_SEARCH_KEYWORD_KEY_PREFIX$userId")
    }
